...
import base64
import os
from removebg import RemoveBg  # Import the Remove.bg library
...
# Initialize Remove.bg API client with your API key
removebg = RemoveBg("1d7hGXxHaqjWbrbV5QfJUBHB", "error.log")
...
def index():
    if request.method == 'POST':
        ...
        if file:
            try:
                # Remove background from the image file
                output_img = removebg.remove_background_from_img_file(file)
                # Convert the output image to base64 string
                encoded_img = base64.b64encode(output_img.getvalue()).decode('utf-8')
                
                # Update existing image in the database with the processed image
                current_user_image = Image.query.filter_by(id=current_user.id).first()
                if current_user_image:
                    current_user_image.data = encoded_img
                    db.session.commit()
                    flash('Background removed and image updated successfully!')
                else:
                    flash('No existing image found to update!')
            except Exception as e:
                flash('Error removing background and updating image: ' + str(e))
